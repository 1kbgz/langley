...
from langley.models import AgentProfile, AuditEntry, CheckpointData, _new_id, _now
from langley.store import StateStore
from langley.transport import MessageTransport
...
class AgentProcessManager:
    ...
    def poll(self) -> list[AgentInfo]:
        ...
        for agent_id, info in items:
            ...
            if (
                info.status == AgentStatus.RUNNING
                and info.last_heartbeat is not None
                and (_now() - info.last_heartbeat) > self._heartbeat_timeout
            ):
                with self._lock:
