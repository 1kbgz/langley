...
class TestSqliteStateStoreCheckpoints:
    ...
    def test_load_returns_latest(self, store):
        store.save_checkpoint(
            CheckpointData(agent_id="a1", tenant_id="t1", state=b"v1", sequence=1, id="cp1")
        )
        store.save_checkpoint(
            CheckpointData(agent_id="a1", tenant_id="t1", state=b"v2", sequence=2, id="cp2")
        )
        store.save_checkpoint(
            CheckpointData(agent_id="a1", tenant_id="t1", state=b"v3", sequence=3, id="cp3")
        )
        loaded = store.load_checkpoint("a1")
        assert loaded is not None
    ...
    def test_list_checkpoints(self, store):
        for i in range(5):
            store.save_checkpoint(
                CheckpointData(
                    agent_id="a1", tenant_id="t1", state=f"s{i}".encode(), sequence=i
                )
            )
    ...
    def test_delete_all_checkpoints(self, store):
        for i in range(3):
            store.save_checkpoint(
                CheckpointData(agent_id="a1", tenant_id="t1", state=b"x", sequence=i)
            )
    ...
    def test_delete_keep_latest(self, store):
        for i in range(5):
            store.save_checkpoint(
                CheckpointData(agent_id="a1", tenant_id="t1", state=f"s{i}".encode(), sequence=i)
            )
    ...
    def test_binary_state_data(self, store):
        ...
        store.save_checkpoint(
            CheckpointData(agent_id="a1", tenant_id="t1", state=binary_data, sequence=1)
        )
        loaded = store.load_checkpoint("a1")
        assert loaded is not None
    ...
    def test_agents_isolated(self, store):
        store.save_checkpoint(
            CheckpointData(agent_id="a1", tenant_id="t1", state=b"state-a1", sequence=1)
        )
        store.save_checkpoint(
            CheckpointData(agent_id="a2", tenant_id="t1", state=b"state-a2", sequence=1)
        )
