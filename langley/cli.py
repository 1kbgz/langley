...
def _api_request(base: str, method: str, path: str, body: dict | None = None) -> dict:
    ...
    try:
        ...
        print(f"[langley] Error connecting to {base}: {e}", file=sys.stderr)
        sys.exit(1)
...
def cmd_up(args: argparse.Namespace) -> int:
    ...
    print(f"[langley] Running at http://{host}:{port}")
    print("[langley] Press Ctrl+C to stop.")
...
def cmd_dev(args: argparse.Namespace) -> int:
    ...
    if not args.ui_only:
        ...
        print(f"[langley] API server running at http://{host}:{api_port}")

    if not args.api_only:
        if js_dir is not None:
            print(f"[langley] Starting JS rebuild watcher in {js_dir}")
            js_proc = _start_js_watch(js_dir)
        else:
            print("[langley] JS source directory not found, skipping JS dev watcher")

    if api_server is None and js_proc is None:
        print("[langley] Nothing to start", file=sys.stderr)
        return 1

    print("[langley] Dev servers running. Press Ctrl+C to stop.")
...
def cmd_agent_list(args: argparse.Namespace) -> int:
    ...
    if not data:
        ...
        return 0
    # Simple table output
    print(f"{'AGENT ID':<36}  {'STATUS':<10}  {'PROFILE':<20}  {'PID':<8}  {'UPTIME'}")
    for a in data:
        uptime = f"{a.get('uptime_seconds', 0):.0f}s"
        pid = str(a.get("pid") or "-")
        print(f"{a['agent_id']:<36}  {a['status']:<10}  {a.get('profile_name', ''):<20}  {pid:<8}  {uptime}")
    return 0
...
def cmd_agent_launch(args: argparse.Namespace) -> int:
    ...
    print(f"Launched agent {data['agent_id']} (status: {data['status']})")
    return 0
